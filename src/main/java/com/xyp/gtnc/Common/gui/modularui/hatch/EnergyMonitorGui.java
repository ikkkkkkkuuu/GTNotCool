...
import com.xyp.gtnc.Common.gui.modularui.GTNCListWidget;
import com.xyp.gtnc.Common.machines.basicMachine.monitor.EnergyMonitor;
import com.xyp.gtnc.Common.machines.basicMachine.monitor.EnergyMonitorHighlightTarget;
import com.xyp.gtnc.Common.machines.basicMachine.monitor.EnergyMonitorMode;
import com.xyp.gtnc.Common.machines.basicMachine.monitor.EnergyMonitorRowSnapshot;
import com.xyp.gtnc.Common.machines.basicMachine.monitor.EnergyMonitorSnapshot;
import com.xyp.gtnc.Common.machines.basicMachine.monitor.EnergyMonitorSummarySnapshot;
...
public class EnergyMonitorGui extends MTETieredMachineBlockBaseGui<EnergyMonitor> {
    ...
    private IWidget createOwnerLine(PanelSyncManager syncManager) {
        ...
        return IKey.dynamic(
            () -> buildTranslatedLine("gtnc.energy_monitor.owner", EnumChatFormatting.AQUA + ownerSyncer.getValue()))
            .asWidget()
        ...
        return Flow.row()
            ...
            .child(IKey.dynamic(() -> {
                ...
                return buildTranslatedLine(
                    "gtnc.energy_monitor.total_energy",
    ...
    private IWidget createAverageLine() {
        return IKey.dynamic(() -> {
            EnergyMonitorSummarySnapshot summary = machine.getSummarySnapshot();
            String key = summary.isOutputMode() ? "gtnc.energy_monitor.average_output"
                : "gtnc.energy_monitor.average_input";
    ...
    private IWidget createEstimatedTimeLine() {
        return IKey.dynamic(() -> {
            EnergyMonitorSummarySnapshot summary = machine.getSummarySnapshot();
            String key = summary.isOutputMode() ? "gtnc.energy_monitor.estimated_empty"
                : "gtnc.energy_monitor.estimated_full";
            return buildTranslatedLine(
                key,
    ...
    private IWidget createStatisticsLine(PanelSyncManager syncManager) {
        ...
        return Flow.row()
            ...
            .crossAxisChildPadding(1)
            .child(
                IKey.str(StatCollector.translateToLocal("gtnc.energy_monitor.statistics"))
                    .asWidget()
                    .textAlign(Alignment.CenterLeft)
                    .maxWidth(TERMINAL_TEXT_WIDTH))
    ...
    private IWidget createLoadMoreHint() {
        return IKey.str(EnumChatFormatting.YELLOW + StatCollector.translateToLocal("gtnc.energy_monitor.scroll_more"))
            .asWidget()
            .textAlign(Alignment.CenterLeft)
            .maxWidth(TERMINAL_TEXT_WIDTH)
            .fullWidth();
    ...
    protected UITexture getLogoTexture() {
        return GTNCGuiTextures.PICTURE_GODFORGE_LOGO;
    }
        ...
        for (EnergyMonitorMode mode : modes) {
            ...
            holder.child(
                new ButtonWidget<>().background(IDrawable.EMPTY)
                    ...
                    .tooltipBuilder(tooltip -> tooltip.addLine(IKey.lang("gtnc.energy_monitor.mode_hint")))
                    .tooltipShowUpTimer(TOOLTIP_DELAY)
    ...
    private static String translateIfNeeded(String value) {
        ...
        return value.startsWith("gtnc.energy_monitor.") ? StatCollector.translateToLocal(value) : value;
    }
    ...
    public class MonitoringListWidget extends GTNCListWidget<IWidget, MonitoringListWidget> {
