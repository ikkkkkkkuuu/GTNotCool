...
public class GTNCMultiBlockBaseGui<T extends MTEMultiBlockBase> extends MTEMultiBlockBaseGui<T> {
    ...
    protected ModularPanel createUpgradeTreePanel(PanelSyncManager sm, ModularPanel parent,
        ...
        List<ItemStack> costs = ((GTNCMultiBlockBase<?>) multiblock).getUpgradeCosts();
        // Sync paid indices from machine → survives game restart
        paidCostIndices.clear();
        paidCostIndices.addAll(((GTNCMultiBlockBase<?>) multiblock).paidUpgradeCostIndices);
        // Keep client in sync with server via bitmask sync value
        sm.syncValue("paidBits", new IntSyncValue(() -> {
            int bits = 0;
            for (int idx : ((GTNCMultiBlockBase<?>) multiblock).paidUpgradeCostIndices) {
    ...
    protected Flow buildCostRow(List<ItemStack> costs, int index) {
        return Flow.row()
            ...
            // Shown when there is no extra cost for this slot
            .child(
                GTGuiTextures.BUTTON_STANDARD_DISABLED.asWidget()
                    .size(18)
                    .setEnabledIf($ -> index >= costs.size()))
            // Shown when there is a cost — item icon with NEI lookup
            .child(new SlotLikeButtonWidget(() -> costs.get(index)).onMousePressed(d -> {
                ...
                .setEnabledIf($ -> index < costs.size()))
            // Amount text — shown while cost exists and not yet paid
            .child(IKey.dynamic(() -> {
            ...
            // Green checkmark — shown when cost is fully paid
            .child(
    ...
    protected void performUpgrade(ItemStackHandler handler) {
        GTNCMultiBlockBase<?> base = (GTNCMultiBlockBase<?>) multiblock;
        ...
        // Only process a single unpaid cost that matches the input
        ...
        for (int idx = 0; idx < costs.size(); idx++) {
            ...
            if (remaining > 0) continue; // Not enough of this cost

            // Consume this cost from input slots
            foundMatch = true;
            ...
            break; // Only one cost per submission
        }
    ...
    protected void onUpgradeComplete() {
        ((GTNCMultiBlockBase<?>) multiblock).onUpgradeComplete();
    }
