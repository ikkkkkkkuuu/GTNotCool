 ...
 * 2. 注入 GregTech 运行时语言表
 * 3. 注入 Forge 的 zh_CN 中文语言表
 *
 * 不检查当前游戏语言，避免注册阶段语言尚未初始化，
 * 导致正式游戏中翻译没有被注册。
 */
public class GTNCLanguageLoader {
    ...
    private static void addWerkstoffLocalization(Werkstoff werkstoff, String zhName, boolean isItemPipe) {
        ...
        if (werkstoff == null || zhName == null || zhName.isEmpty()) {
            return;
        }
        ...
        String materialName = unlocalizedName.replace(" ", "");

        /*
         * Werkstoff 材料名称。
         */
        storeTranslation("Material." + materialName, zhName);
    ...
    /**
     * 写入 GregTech 配置和运行时语言表，
     * 同时注入 Forge 的 zh_CN 中文语言表。
     */
    private static synchronized void storeTranslation(String key, String text) {
        ...
        try {
            ...
            LanguageRegistry.instance()
                .injectLanguage(LANGUAGE_ZH_CN, tempMap);
        } finally {
            /*
             * 防止临时语言表内容污染后续翻译注册。
             */
