...
public class BoxRecipe {
    ...
    public IAEStack<?>[] transInputsToAE2Stuff() {
        IAEStack<?>[] stacks = new IAEStack<?>[FinalItemInput.size() + FinalFluidInput.size()];
        int i = 0;
        ...
        for (FluidStack fluid : FinalFluidInput) {
            stacks[i] = AEFluidStack.create(fluid);
            i++;
        }
        return stacks;
    }

    public IAEStack<?>[] transOutputsToAE2Stuff(String itemKey, String fluidKey) {
        ...
        if (fluidKey.equals("")) {
            for (FluidStack fluid : FinalFluidOutput) {
                stacks.add(AEFluidStack.create(fluid));
            }
            ...
            for (String s : var1) {
                if (s.contains("-")) {
                    ...
                    int end = Integer.parseInt(var2[1]);
                    for (int i = start - 1; i < end; i++) {
                        stacks.add(AEFluidStack.create(FinalFluidOutput.get(i)));
                    }
                } else stacks.add(AEFluidStack.create(FinalFluidOutput.get(Integer.parseInt(s) - 1)));
            }
        }
        return stacks.toArray(new IAEStack<?>[0]);
    }

    public NBTTagCompound RecipeToAE2UltimatePattern(String itemKey, String fluidKey) {
        final NBTTagCompound encodedValue = new NBTTagCompound();
        encodedValue.setTag("in", writeAEStackArray(transInputsToAE2Stuff()));
        encodedValue.setTag("out", writeAEStackArray(transOutputsToAE2Stuff(itemKey, fluidKey)));
        encodedValue.setBoolean("crafting", false);
        encodedValue.setBoolean("substitute", false);
        encodedValue.setBoolean("beSubstitute", false);
        return encodedValue;
    ...
    private static NBTTagList writeAEStackArray(IAEStack<?>[] stacks) {
        final NBTTagList tagList = new NBTTagList();
        for (IAEStack<?> stack : stacks) {
            final NBTTagCompound tag = new NBTTagCompound();
            Platform.writeStackNBT(stack, tag, true);
