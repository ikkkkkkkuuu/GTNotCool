...
public class BoxModule {
    ...
    public static int[] transMachinesToModule(BoxRoutings routing) {
        if (routing == null || routing.RoutingMachine == null) {
            return new int[] { 14, 0 };
        }
        ...
        return switch (unlocalizedName.substring(17)) {
            ...
            case "quantumforcetransformer.controller.tier.single", "frf", "industrialmassfab.controller.tier.single" -> new int[] {
                10, 1 };
            default -> {
                int[] recipeMapModule = transRecipeMapToModule(routing);
                if (recipeMapModule != null) {
                    yield recipeMapModule;
                }
    ...
    private static int[] transRecipeMapToModule(BoxRoutings routing) {
        if (!(getMetaTileEntity(routing.RoutingMachine) instanceof MTEMultiBlockBase machine)) {
            return null;
        }
        RecipeMap<?> recipeMap = (machine instanceof IBoxable boxable) ? boxable.getRealRecipeMap(machine)
        ...
        if (isAnyRecipeMap(recipeMap, RecipeMaps.crackingRecipes, RecipeMaps.distillationTowerRecipes)) {
            return new int[] { 2, 0 };
        }
        if (isAnyRecipeMap(
            recipeMap,
        ...
        if (isAnyRecipeMap(recipeMap, RecipeMaps.fluidHeaterRecipes, RecipeMaps.neutroniumCompressorRecipes)) {
            return new int[] { 5, 0 };
        }
        if (isAnyRecipeMap(
            recipeMap,
        ...
        if (isAnyRecipeMap(recipeMap, BartWorksRecipeMaps.circuitAssemblyLineRecipes, RecipeMaps.pcbFactoryRecipes)) {
            return new int[] { 4, 1 };
        }
        if (recipeMap == RecipeMaps.replicatorRecipes) {
            return new int[] { 0, 1 };
        }
        if (recipeMap == GoodGeneratorRecipeMaps.preciseAssemblerRecipes) {
            return new int[] { 1, 1 };
        }
        ...
        if (isAnyRecipeMap(recipeMap, RecipeMaps.implosionRecipes, RecipeMaps.spaceProjectFakeRecipes)) {
            return new int[] { 9, 1 };
        }
        if (isAnyRecipeMap(recipeMap, RecipeMaps.massFabFakeRecipes, RecipeMaps.amplifierRecipes)) {
            return new int[] { 10, 1 };
        }
        return null;
    }

    private static boolean isAnyRecipeMap(RecipeMap<?> recipeMap, RecipeMap<?>... recipeMaps) {
        for (RecipeMap<?> candidate : recipeMaps) {
            if (recipeMap == candidate) {
                return true;
            }
        }
        return false;
